...
const DarkButton = () => {
  ...
  const defaultDarken = useMemo(() => {
    ...
    return localStorage.getItem('VisComponents_dark_theme') || colorScheme;
  }, []);
  ...
  return (
    <Switch
      ...
      onChange={(check) => {
        toggle();
        if (!check) {
          localStorage.setItem('VisComponents_dark_theme', 'light');
        } else {
          localStorage.setItem('VisComponents_dark_theme', 'dark');
        }
      }}
    />
  );
...
const LayoutPage = ({ children, ...props }: IRouteComponentProps) => {
  ...
  const title = useMemo(() => {
    ...
    if (!context.meta.title) {
      return 'VisComponents - 模板组件';
    }
    return `${context.meta.title} - VisComponents`;
  }, [context]);
        ...
        <Layout {...props}>
          <div>
            ...
            <div
              ...
              key="VisComponents_dark_theme_view"
              className="VisComponents_dark_theme_view"
            >
