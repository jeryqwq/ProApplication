...
export interface PlayerProps {
  ...
  statusChange?: (_: 'SUSPEND' | 'RESTART' | 'START', config: PlayerProps ) => void;
  [key: any]: any;
}
...
function handleSpeak ({ speakText, statusChange, loop }) {
  !loop && speechSynthUtterance && speechSynthUtterance.removeEventListener('end', speakFn)
  audioInstance && audioInstance.state !== 'closed' && audioInstance.suspend()
  if (speechSynthesis.speaking) {
    speechSynthesis.pause()
    statusChange && statusChange('SUSPEND')
    speechSynthUtterance && speechSynthUtterance.removeEventListener('end', speakFn)
...
function suspendOtherAud (src) { // 避免多个音频同时播放
...
const audioPlayer: PlayerType = {
  ...
  update (el, { oldValue: { loop: _loop, type: _type, speakText: _speakText, src: _src, auto: _auto }, value: { type, src, speakText, loop = false, auto, statusChange } }) {
    if (!speakText && !src) return
    if (loop === _loop) { // 条件更新，
      if (speakText && type === 'SPEAK' && type === _type && auto === _auto) { // 循环参数&& 文字内容不变 && 类型不变 不更新
        if (_speakText === speakText) return
      }
      if (src && type === 'AUDIO' && type === _type && auto === _auto) { // 循环参数&& url链接 && 类型不变  => 不更新
        if (src === _src) return
      }
    }
    closeAllVoice()
